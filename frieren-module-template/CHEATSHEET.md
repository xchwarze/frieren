...
```json
{
  ...
  "autoload-dev": {
    "psr-4": {
      ...
      "frieren\\modules\\{name}\\Tests\\": "tests/"
    }
  },
...
`tests/Support/DispatchesControllers.php`):
```php
...
instead, as the `file('/proc/stat')` mock above does — see `tests/ModuleOpenWrtHelperTest.php`
...
OS boundary — see `tests/ModuleOpenWrtHelperTest.php` and `tests/DemoControllerTest.php` for
...
Component test — `src/__tests__/StateDemoCard.test.jsx` renders the *real* component (only its
...
```js
// src/__tests__/queryKeys.test.js
